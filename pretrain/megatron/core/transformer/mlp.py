...
class vanillaMLP(MegatronModule):
    """
...
class MLP(MegatronModule):
    ...
    def __init__(
        ...
        self.moe_router = build_module(
            ...
            skip_bias_add=True,
            is_expert=is_expert,
            tp_comm_buffer_name='moe_router',
        )
    ...
    def forward(self, hidden_states, init_states):
        ...
        router_weight = self.moe_router(hidden_states)[0]
        router_weight = torch.softmax(router_weight, dim=-1, dtype=torch.float32).type_as(router_weight)
        dispatched_input = init_states.reshape(1, -1, self.config.hidden_size).expand(self.num_experts, -1, -1).reshape(-1, self.config.hidden_size)
        token_per_expert = torch.tensor([init_states.size(0) * init_states.size(1)] * self.num_experts, dtype=torch.long)
        expert_output = self.experts(dispatched_input, token_per_expert)[0]
        expert_output = expert_output.reshape(self.num_experts, init_states.size(0), init_states.size(1), -1)
        expert_output = (expert_output * router_weight.permute(2, 0, 1).unsqueeze(-1)).sum(dim=0)
